...
var eventResponderContext = {
  dispatchEvent: function (possibleEventObject, listener, _ref) {
    ...
    }
    {
      var showWarning = function (name) {
        warning$1(false, '%s is not available on event objects created from event responder modules (React Flare).', name);
      };
  ...
  getEventPointerType: function (event) {
    validateResponderContext();
  ...
  getEventCurrentTarget: function (event) {
    ...
    while (currentTarget.parentNode && currentTarget.parentNode.nodeType === Node.ELEMENT_NODE && isTargetWithinEventComponent(currentTarget.parentNode)) {
...
var familiesByType = null;
// $FlowFixMe Flow gets confused by a WeakSet feature check below.
var failedBoundaries = null;
var fibersDeletedDuringHotReload = null;
...
function resolveFunctionForHotReloading(type) {
  {
    ...
    var family = familiesByType.get(type);
    if (family === undefined) {
      return type;
    }
...
function isCompatibleFamilyForHotReloading(fiber, element) {
  {
    ...
    if (needsCompareFamilies) {
      ...
      var prevFamily = familiesByType.get(prevType);
      if (prevFamily !== undefined && prevFamily === familiesByType.get(nextType)) {
        return true;
      }
    }
    return false;
...
function markDeletedFiberForHotReloading(fiber) {
  {
...
function scheduleHotUpdate(hotUpdate) {
  {
    // TODO: warn if its identity changes over time?
    familiesByType = hotUpdate.familiesByType;

    var affectedSubtrees = new Set();
    var _root = hotUpdate.root,
        _staleFamilies = hotUpdate.staleFamilies,
        _updatedFamilies = hotUpdate.updatedFamilies;

    flushPassiveEffects();

    var deletedFiberParents = fibersDeletedDuringHotReload = new Map();
    try {
      flushSync(function () {
        scheduleFibersWithFamiliesRecursively(_root.current, _updatedFamilies, _staleFamilies, affectedSubtrees);
      });
    } finally {
      fibersDeletedDuringHotReload = null;
    }

    var _hostNodesForVisualFeedback = findHostNodesForVisualFeedback(affectedSubtrees, deletedFiberParents);
    return { hostNodesForVisualFeedback: _hostNodesForVisualFeedback };
  }
}

function scheduleFibersWithFamiliesRecursively(fiber, updatedFamilies, staleFamilies, affectedSubtrees) {
  {
    var alternate = fiber.alternate,
        child = fiber.child,
    ...
    if (candidateType !== null) {
      var family = familiesByType.get(candidateType);
      if (family !== undefined) {
    ...
    if (sibling !== null) {
      scheduleFibersWithFamiliesRecursively(sibling, updatedFamilies, staleFamilies, affectedSubtrees);
    }
  }
}

function findHostNodesForVisualFeedback(fibers, deletedFiberParents) {
  {
    var hostNodeSet = new Set();
    fibers.forEach(function (fiber) {
      // Find closest non-deleted Fiber from here.
      while (deletedFiberParents.has(fiber)) {
        var parent = deletedFiberParents.get(fiber);
        if (parent === undefined) {
      ...
      var hostFibers = findAllCurrentHostFibers(fiber);
...
function getHostParentInstance(fiber) {
  {
...
function commitUnmount(current$$1) {
  ...
  {
    markDeletedFiberForHotReloading(current$$1);
  }
...
function commitRootImpl(root) {
  ...
  (function () {
    if (!(finishedWork !== root.current)) {
      {
        throw ReactError('Cannot commit the same tree as before. This error is likely caused by a bug in React. Please file an issue.');
      }
    }
  })();
